...
import com.safecar.platform.workshopOps.domain.services.WorkshopOrderQueryService;
...
public class WorkshopOpsWorkshopOrdersController {
    ...
    public ResponseEntity<List<WorkOrderResource>> getByWorkshop(
        ...
        var workshop = new com.safecar.platform.workshopOps.domain.model.valueobjects.WorkshopId(workshopId,
                "Workshop " + workshopId);
        com.safecar.platform.workshopOps.domain.model.valueobjects.WorkOrderStatus st = null;
        if (status != null)
            st = com.safecar.platform.workshopOps.domain.model.valueobjects.WorkOrderStatus.valueOf(status);
    ...
    public ResponseEntity<WorkOrderResource> patchClose(@PathVariable Long id) {
        var command = new com.safecar.platform.workshopOps.domain.model.commands.CloseWorkOrderCommand(id);
        commandService.handle(command);
    ...
    public ResponseEntity<Void> postAddAppointment(@PathVariable Long id,
            @Valid @RequestBody com.safecar.platform.workshopOps.interfaces.rest.resources.CreateAppointmentResource resource) {
        var slot = new com.safecar.platform.workshopOps.domain.model.valueobjects.ServiceSlot(resource.startAt(),
                resource.endAt());
        var command = new com.safecar.platform.workshopOps.domain.model.commands.AddAppointmentToWorkOrderCommand(id,
                slot);
        commandService.handle(command);
        return ResponseEntity.status(HttpStatus.CREATED).build();
    }
